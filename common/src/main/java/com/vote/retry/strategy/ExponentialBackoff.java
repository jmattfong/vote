...
/**
 * When all retries fail, the default strategy will take about 3 seconds plus the time taken by the task itself.
 */
@Value
@Builder
public class ExponentialBackoff implements RetryStrategy {
    ...
    public static final double DEFAULT_EXP_BASE = 2;
    ...
    @Builder.Default
    public double exponentBase = DEFAULT_EXP_BASE;
    ...
    public int getRetryDelayMs(int retryCount) {
        // subtract 1 because retryCount starts at 1
        int exponent = retryCount - 1;
        return (int) Math.pow(exponentBase, exponent) * initialBackoffAmountMs + random.nextInt(maxJitterMs);
    }
}
